...
from keras.layers import Input, Lambda, Activation, Conv2D, MaxPooling2D, ZeroPadding2D, Reshape, Concatenate,BatchNormalization, Add, Conv2DTranspose
...
from utils_1.keras_layer_DecodeDetectionsFast import DecodeDetectionsFast

# import tensorflow as tf


def ssd_300_mobilenet_V2(layers,
            mode,
            weights_base,
            image_size,
            n_classes,
    ...
    from models_madhu.mobilenet_v2_for_SSD import MobileNetV2, LiteConv 
    print(x1.shape[1:])


    # block_17_conv_1280, block_13_conv_160 = MobileNetV2(input_shape=x1.shape[1:],include_top=False)#,weights='imagenet')
    block_13_conv_160, block_17_conv_1280, x_flow = MobileNetV2(input_tensor=x1, include_top=False, weights=weights_base)#,weights='imagenet')
    prior_1_feats = block_13_conv_160
    prior_2_feats = block_17_conv_1280

    # prior_2_feats = Conv2D(1280, kernel_size=1, strides=(2,2), use_bias=False, name='Conv_prior_2_feats')(block_17_conv_1280) # this might not be there
    # x_flow = BatchNormalization(epsilon=1e-3, momentum=0.999, name='Conv_prior_2_feats_bn')(prior_2_feats) # this might not be there
    # x_flow = Activation('relu',name = 'Conv_prior_2_feats_relu')(prior_2_feats) # this might not be there


    x_flow, prior_3_feats = LiteConv(x_flow, "block_19", 512)
    x_flow, prior_4_feats = LiteConv(x_flow, "block_20", 256)
    x_flow, prior_5_feats = LiteConv(x_flow, "block_21", 256)
    x_flow, prior_6_feats = LiteConv(x_flow, "block_22", 128)

    # conv4_3_norm , fc7, conv6_2 = MobileNet(input_tensor=x1,layers=layers)


    # training_flag = True

    # if(layers == "head" or layers == "all"):
    #     training_flag = True
    #     print "Training the heads of model"
    # else:
    #     training_flag = False
    #     print "Training off for the heads"


    # fc7_2 = Conv2DTranspose(256,(2, 2), strides=2,padding='valid', kernel_initializer='he_normal',kernel_regularizer=l2(l2_reg), name='fc7_2',trainable=training_flag)(fc7)
    # fc7_2 = BatchNormalization(name='norm_fc7')(fc7_2)
    # fc7_2 = Activation('relu', name='relu_fc7_2')(fc7_2)
   
    # print ('fc7_2 shape', fc7_2.shape)


    # print conv4_3_norm.shape
    
    # conv6_2 = MobileNet(input_tensor=x1)
    # print conv6_2.shape
    # model = Model(inputs=x, outputs=conv6_2)
    # return model




    # print ('conv4_3_norm shape', conv4_3_norm.shape)
    # print ('fc7 shape', fc7.shape)
    # print ('conv6_2 shape', conv6_2.shape)

    # conv7_1 = Conv2D(128, (1, 1), padding='same', kernel_initializer='he_normal',trainable=training_flag,
    #     kernel_regularizer=l2(l2_reg), name='conv7_1')(conv6_2)
    # conv7_1 = BatchNormalization(name='norm_7_1')(conv7_1)
    # conv7_1 = Activation('relu', name='relu_conv7_1')(conv7_1)
    # print('srjm', conv7_1.shape)
    # conv7_1 = ZeroPadding2D(padding=((1, 1), (1, 1)), name='conv7_padding')(conv7_1)
    # conv7_2 = Conv2D(256, (3, 3), strides=(2, 2), padding='valid', kernel_initializer='he_normal',
    #     kernel_regularizer=l2(l2_reg), name='conv7_2',trainable=training_flag)(conv7_1)
    # conv7_2 = BatchNormalization(name='norm_7_2')(conv7_2)
    ...
    #     kernel_regularizer=l2(l2_reg), name='conv8_1',trainable=training_flag)(conv7_2)
    # conv8_1 = BatchNormalization(name='norm_8_1')(conv8_1)
    ...
    #     kernel_regularizer=l2(l2_reg), name='conv8_2',trainable=training_flag)(conv8_1)
    # conv8_2 = BatchNormalization(name='norm_8_2')(conv8_2)
    # conv8_2 = Activation('relu', name='relu_conv8_2')(conv8_2)

    # print ('conv8_2 shape', conv8_2.shape)

    # conv9_1 = Conv2D(128, (1, 1), padding='same', kernel_initializer='he_normal',
    #     kernel_regularizer=l2(l2_reg), name='conv9_1',trainable=training_flag)(conv8_2)
    # conv9_1 = BatchNormalization(name='norm_9_1')(conv9_1)
    ...
    #     kernel_regularizer=l2(l2_reg), name='conv9_2',trainable=training_flag)(conv9_1)
    # conv9_2 = BatchNormalization(name='norm_9_2')(conv9_2)
    ...
    # fc7_norm = L2Normalization(gamma_init=10, name='fc7_2_norm')(fc7_2)
    # conv4_3_norm = L2Normalization(gamma_init=20, name='conv4_3_norm')(conv4_3_norm)
    # conv4_3_norm = Add(name='conv4_fc7_element_wise_sum')([conv4_3_norm,fc7_norm])

    conv4_3_norm = prior_1_feats
    fc7 = prior_2_feats
    conv6_2 = prior_3_feats
    conv7_2 = prior_4_feats
    conv8_2 = prior_5_feats
    conv9_2 = prior_6_feats
    ...
    conv4_3_norm_mbox_conf = Conv2D(n_boxes[0] * n_classes, (3, 3), padding='same', kernel_initializer='he_normal',
                                    kernel_regularizer=l2(l2_reg), name='conv4_3_norm_mbox_conf',trainable=training_flag)(conv4_3_norm)
    fc7_mbox_conf = Conv2D(n_boxes[1] * n_classes, (3, 3), padding='same', kernel_initializer='he_normal',
                           kernel_regularizer=l2(l2_reg), name='fc7_mbox_conf',trainable=training_flag)(fc7)
    conv6_2_mbox_conf = Conv2D(n_boxes[2] * n_classes, (3, 3), padding='same', kernel_initializer='he_normal',
                               kernel_regularizer=l2(l2_reg), name='conv6_2_mbox_conf',trainable=training_flag)(conv6_2)
    conv7_2_mbox_conf = Conv2D(n_boxes[3] * n_classes, (3, 3), padding='same', kernel_initializer='he_normal',
                               kernel_regularizer=l2(l2_reg), name='conv7_2_mbox_conf',trainable=training_flag)(conv7_2)
    conv8_2_mbox_conf = Conv2D(n_boxes[4] * n_classes, (3, 3), padding='same', kernel_initializer='he_normal',
                               kernel_regularizer=l2(l2_reg), name='conv8_2_mbox_conf',trainable=training_flag)(conv8_2)
    conv9_2_mbox_conf = Conv2D(n_boxes[5] * n_classes, (3, 3), padding='same', kernel_initializer='he_normal',
                               kernel_regularizer=l2(l2_reg), name='conv9_2_mbox_conf',trainable=training_flag)(conv9_2)
    # We predict 4 box coordinates for each box, hence the localization predictors have depth `n_boxes * 4`
    # Output shape of the localization layers: `(batch, height, width, n_boxes * 4)`
    conv4_3_norm_mbox_loc = Conv2D(n_boxes[0] * 4, (3, 3), padding='same', kernel_initializer='he_normal',
                                   kernel_regularizer=l2(l2_reg), name='conv4_3_norm_mbox_loc',trainable=training_flag)(conv4_3_norm)
    fc7_mbox_loc = Conv2D(n_boxes[1] * 4, (3, 3), padding='same', kernel_initializer='he_normal',
                          kernel_regularizer=l2(l2_reg), name='fc7_mbox_loc',trainable=training_flag)(fc7)
    conv6_2_mbox_loc = Conv2D(n_boxes[2] * 4, (3, 3), padding='same', kernel_initializer='he_normal',
                              kernel_regularizer=l2(l2_reg), name='conv6_2_mbox_loc',trainable=training_flag)(conv6_2)
    conv7_2_mbox_loc = Conv2D(n_boxes[3] * 4, (3, 3), padding='same', kernel_initializer='he_normal',
                              kernel_regularizer=l2(l2_reg), name='conv7_2_mbox_loc',trainable=training_flag)(conv7_2)
    conv8_2_mbox_loc = Conv2D(n_boxes[4] * 4, (3, 3), padding='same', kernel_initializer='he_normal',
                              kernel_regularizer=l2(l2_reg), name='conv8_2_mbox_loc',trainable=training_flag)(conv8_2)
    conv9_2_mbox_loc = Conv2D(n_boxes[5] * 4, (3, 3), padding='same', kernel_initializer='he_normal',
                              kernel_regularizer=l2(l2_reg), name='conv9_2_mbox_loc',trainable=training_flag)(conv9_2)
