...
def SSD(input_shape=(300,300,3), num_classes=81):
    """SSD300 architecture.
    ...
    mobilenetv2_input_shape=(300,300,3)#(224,224,3)

    Input0 = Input(input_shape)
    mobilenetv2=MobileNetV2(input_shape=mobilenetv2_input_shape,include_top=False, weights='imagenet')
    FeatureExtractor=Model(inputs=mobilenetv2.input, outputs=mobilenetv2.get_layer('block_12_add').output)#.get_layer('res_connect_12').output)
    ...
    x,pwconv3 = _isb4conv13(x, filters=160, alpha=alpha, stride=1,expansion=6, block_id=13) # stride should be 2 as per the model and <https://github.com/chuanqi305/MobileNetv2-SSDLite>
    #x=get_3rd_layer_output([x,1])[0]
    x = _inverted_res_block(x, filters=160, alpha=alpha, stride=1,expansion=6, block_id=14)
    x = _inverted_res_block(x, filters=160, alpha=alpha, stride=1,expansion=6, block_id=15)
    x = _inverted_res_block(x, filters=320, alpha=alpha, stride=1,expansion=6, block_id=16)
    x, pwconv4 = Conv(x, 1280)
    x, pwconv5 = LiteConv(x, "block_19", 512)
    x, pwconv6 = LiteConv(x, "block_20", 256)
    x, pwconv7 = LiteConv(x, "block_21", 256)
    x, pwconv8 = LiteConv(x, "block_22", 128)

    # aspect ratios here fed into are : [2] or [2, 3]
    # default aspect ratio is [1.0]
    # flips of all these aspect ratios
    # so filnally ars are [1.0, 2.0, 3.0, 1 / 2.0, 1 / 3.0]
